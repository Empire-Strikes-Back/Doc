...
(defonce program-data-dirpath (or
                               (some-> (System/getenv "Doc_PATH")
                                       (.replaceFirst "^~" (System/getProperty "user.home")))
                               (.getCanonicalPath ^File (clojure.java.io/file (System/getProperty "user.home") "Doc"))))

(defonce state-file-filepath (.getCanonicalPath ^File (clojure.java.io/file program-data-dirpath "Doc.edn")))
...
(def ^:const jframe-title "why are things so heavy in the future?")
...
(defn -main
  ...
  #_(alter-var-root #'*ns* (constantly (find-ns 'Doc.main)))
    ...
    (SwingUtilities/invokeLater
     (reify Runnable
       (run [_]
         ...
         (alter-var-root #'Doc.main/jframe (constantly jframe))
